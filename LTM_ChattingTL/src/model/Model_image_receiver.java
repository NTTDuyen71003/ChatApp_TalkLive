 ...
 * @author DD
 */
public class Model_image_receiver {
    ...
    public EventImageReceiver getEvent() {
        return event;
    }

    public void addEvent(EventImageReceiver event) {
        this.event = event;
    }
    

    public Model_image_receiver(int fileID, Socket socket, EventImageReceiver event) {
        this.fileID = fileID;
        this.socket = socket;
        this.event = event;
    }

    public Model_image_receiver() {
    }
    ...
    private EventImageReceiver event;
    private final String PATH_FILE = "client_img/";
    ...
    public void startSaveFile() throws IOException, JSONException {
        Model_request_image data = new Model_request_image(fileID, accFile.length());
            ...
            public void call(Object... os) {
                try {
                    if (os.length > 0) {
                        ...
                        //  remove list
                        Service.getInstance().fileReceiveFinish(Model_image_receiver.this);
                    }
                } catch (IOException | JSONException e) {
                    e.printStackTrace();
                }
